...
export function registerPush (condition) {
  return request.post(`${request.getUrl().auth}/api/AnonymousInfo`, {
      body: condition
  })
}
...
export function setUserExtension (condition) {
    return request.post(`${request.getUrl().auth}/api/user/extensions`, {
        method: 'POST',
        body: condition
    })
}
