...
export default {
  ...
  list: () => {
    ...
    return request.get(`${request.getUrl().api}/v2.0/api/message/mymessage`)
  },
  // 客户动态
  ...
  // 阅读消息
  read: (params: ReadConditions): Promise<void> => {
    return request.post(`${request.getUrl().api}/v2.0/api/message/readmessage`, {
      body: params
    })
  },

  // 阅读动态消息
  readDt: (params: ReadConditions): Promise<void> => {
    return request.post(`${request.getUrl().api}/v2.0/api/customerInfo/readcustomerdynamic`, {
      body: params
    })
  },
  // 删除消息
