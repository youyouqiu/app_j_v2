// @ts-ignore
/**
 * Created by Kary on 2019/07/30 15:53.
 */
'use strict';
...
const loanFormula = {
    // 等额本息每月还款金额
    getRepayPerMouPriceAi: function (loanTotal: number, interestRatePerMou: number, totalMouths: number) {
        /**
         * @param loanTotal         总贷款
    ...
    // 等额本息第i个月还款中利息部分
    getRepayInterestPerMouAi: function (i: number, loanTotal: number, interestRatePerMou: number, totalMouths: number): number {
        /**
         * @param i   第(i + 1)个月
...
export interface repayPerMouObj_Type {
    repayPerMouObjAi: {
        repayInterestPerMouArrAi: number[];
        repayPrincipalPerMouArrAi: number[];
        balanceArrAi: number[];
        totalRepaidArrAi: number[];
        totalRepayPerMouArrAi: number[];
    };
    repayPerMouObjAp: {
        repayInterestPerMouArrAp: number[];
        repayPerMouPriceArrAp: number[];
        balanceArrAp: number[];
        totalRepaidArrAp: number[];
        totalRepayPerMouArrAp: number[];
    };
}
export interface detail_Type {
    loanTotal: number;
    totalInterestAi: number;
    totalRepayAi: number;
    repayPerMouAi: number;
    totalInterestAp: number;
    totalRepayPriceAp: number;
    repayPrincipalPerMouAp: number;
    repayPerMouthAp: number;
    decreasePerMouAp: number;
    totalYears: number;
    repaymentTable: repayPerMouObj_Type
}

// 格式化金额，保留两位小数
export const formatFloat = function (input: number, decimals: number): number {
    let formatFloatNumb = Math.round(input * Math.pow(10, decimals)) / Math.pow(10, decimals);
    return +formatFloatNumb.toFixed(decimals);
};

export const calculate = function (loanTotal: number, interestRatePerMou: number, totalMouths: number): detail_Type {

    // 每月需还利息、本金、剩余待还本金、已还总金额
    let repayPerMouObj: repayPerMouObj_Type = {
        // 等额本息
        repayPerMouObjAi: {
    ...
    return {
        loanTotal: formatFloat(loanTotal/10000, 2), //贷款总额
        totalInterestAi: formatFloat(totalInterestAi/10000, 2), //等额本息总还款利息
        totalRepayAi: formatFloat(totalRepayAi, 2), //等额本息总还本带息
        repayPerMouAi: formatFloat(repayPerMouAi, 2), //等额本息月均还本带息
        totalInterestAp: formatFloat(totalInterestAp/10000, 2), //等额本金总还款利息
        totalRepayPriceAp: formatFloat(totalRepayPriceAp, 2), //等额本金总还款金额
        repayPrincipalPerMouAp: formatFloat(repayPrincipalPerMouAp, 2),//等额本金每月所还本金
        repayPerMouthAp: formatFloat(repayPerMouObj.repayPerMouObjAp.repayPerMouPriceArrAp[0] - decreasePerMouAp, 2) ,//等额本金第一个月还款
        decreasePerMouAp: formatFloat(decreasePerMouAp, 2),//之后每个月递减额
        repaymentTable: {
            repayPerMouObjAi: repayPerMouObj.repayPerMouObjAi,//等额本息详情
            repayPerMouObjAp: repayPerMouObj.repayPerMouObjAp,//等额本金详情
        },
        totalYears: totalMouths / 12, // 贷款年限
    }
};
