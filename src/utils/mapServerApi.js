...
export const mapSearch = (lat, lng, query, size = 3, tag) => {
    let url = `http://api.map.baidu.com/place/v2/search?query=${query}&tag=${tag}&radius_limit=true&location=${lat},${lng}&radius=2000&page_size=${size}&page_num=0&scope=2&output=json&ak=BvbGX2GsoWdpmmZAbT4YhjwVNyx0pSFI`
    // console.log(url)
    return request.getPure(url)
        .then(res=>{return res})
}
