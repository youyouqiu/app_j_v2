...
function getDeviceInfo() {
  const deviceinfo = {
    ...
    // Source: 3, // 1 ios 2 android 3 web  todo
...
const request = {
  getUrl: () => {
    const {api, upload, public: common, buryPoint, cqAuth, auth, label, file, authWeb} = global.store.getState().config.requestUrl;
    return {
      api: api,
      upload: upload,
      public: common,
  ...
  get(url, useToken = true, qs, stoken) {
    ...
    let environment = store.getState().config?.environment
    if (environment && environment === 'canary') {
      headers.append('xkj-env-site', 'canary')
    } else {
      headers.append('xkj-env-site', 'prod')
    }
    let token
    if (useToken) {
  ...
  delete(url, useToken = true, qs, stoken) {
    let rc = {}
    // let startTime = Date.now().valueOf()
    const headers = new Headers()
    let startTime = Date.now().valueOf()
  ...
  getPure: url => {
    return fetch(url)
      ...
      .catch(e => ({code: '500', message: e.message || '网络异常', error: e}))
  },
}
