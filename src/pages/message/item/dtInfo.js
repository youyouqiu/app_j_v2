        ...
        if (item.trackType == 1) {
            content = (
                <Text style={newsStyles.contentText}>
                    ...
                    <Text style={newsStyles.nameText} onPress={() => this.gotoBuildingDetail(item.buildingId)}>{item.buildingName}</Text>
                    {`${item.shopName ? '的' : ''}`}
                    <Text style={newsStyles.nameText} onPress={() => this.gotoShopDetail(item.buildingId,item.shopId)}>{item.shopName}</Text>
                </Text>
            )
            ...
            content = (
                <Text style={newsStyles.contentText}>
                    ...
                    <Text style={newsStyles.nameText} onPress={() => this.gotoBuildingDetail(item.buildingId)}>{item.buildingName}</Text>筛选了
                    {item.wordType == 1 ? '价格' : item.wordType == 2 ? '面积' : ''}
