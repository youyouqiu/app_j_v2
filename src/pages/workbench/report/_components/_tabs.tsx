...
class ReportList extends PureComponent<propsTypes & NavigationScreenProps> {
    ...
    state = {
        page: this.props.initPage || 0, // ! 初始tab页
    ...
    handleRenderFooter = (tabsItem: any, page: number) => {
        ...
        let text: JSX.Element = <Text>～ 没有了 ～</Text>;
        if (page === 0) {
            text = tabsItem.length < (totalCount || {}).reportTotal ? (
                <ActivityIndicator/>
            ) : (
                <Text>～ 没有了 ～</Text>
            );
        ...
        if (page === 1) {
            text = tabsItem.length < (totalCount || {}).beltLookTotal ? (
                <ActivityIndicator/>
            ) : (
                <Text>～ 没有了 ～</Text>
            );
        ...
        if (page === 2) {
            text = tabsItem.length < (totalCount || {}).lapseTotal ? (
                <ActivityIndicator/>
            ) : <Text>～ 没有了 ～</Text>;
        }
        return (
                            ...
                            return (
                                ...
                                >
                                    <Text style={page === index ? styles['title-text'] : null}>
                                        {`${item.name} (${newTotalCount[index]})`}
                                    </Text>
                                    <View style={page === index ? styles['title-textBottom'] : null}/>
                                </TouchableOpacity>
                            )
                        })
                    }
                ...
                <View style={{backgroundColor: '#F8F8F8', flex: 1}}>
                    {
                        tabsItem.length === 0 && !tabsItemIsNull[page] ?(
                                ...
                                <FlatList
                                    ...
                                    onEndReached={() => {
                                        isEndReached[page] ? this.endReachedData(page) : null
                                    }}
