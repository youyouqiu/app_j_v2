...
import { View, StyleSheet, Text, TouchableOpacity, Image, TextInput, DeviceEventEmitter, Animated, ScrollView } from 'react-native'
...
const styles = StyleSheet.create({
    ...
    phoneNumber: (fillIn) => ({
        fontSize: scaleSize(28),
        color: fillIn ? '#000' : '#CBCBCB',
    }),
    phoneNumberWrap: (fillIn) => ({
        alignItems: 'center',
        marginLeft: scaleSize(4),
        width: scaleSize(40),
        height: scaleSize(40),
        borderBottomColor: '#CBCBCB',
        borderBottomWidth: !fillIn && scaleSize(2),
    })
})
...
class AddCustom extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            isMan: true,
            phoneList: [{ key: 0, status: false, phone: '' }],
    ...
    copyData = async () => {
        ...
        const phoneList = (dataInfo.customerPhones || []).map((item, index) => ({
            key: index,
            status: /^1[3-9]{1}[\d]{9}$/.test(item.phone),
            phone: item.phone,
            id: item.id
        }))
    ...
    addPhone = () => {
        ...
        phoneList.push({ key: phoneList.length, status: false, phone: '' })
        this.setState({
            phoneList,
        })
    }
    ...
    submit = async () => {
        ...
        if (submitObj.customerName === undefined) {
            Toast.message('请填写必填信息后再提交')
            return
        }
        let regName = /^[\u4E00-\u9FA5\uf900-\ufa2d·s]{1,7}$/,
            regPhone = /^1[3-9]{1}[\d]{9}$/,
            regBanPhone = /^1[3-9][\*|\d]{9}$/,
            regNum = /^[1-9][0-9]{0,3}$/;
        ...
        const result = phoneList.some((item) => {
            if (item.phone.length !== 11) {
                Toast.message('请输入正确的手机号码');
                return true
            }
            else if (!regBanPhone.test(item.phone)) {
                Toast.message('手机号第1、2位需是数字');
                return true
            }
        })
        if (result) return
        ...
        if (id) {
            ...
            try {
                ...
                if (res.code === '0') {
                    // form.resetFields()
                    //页面进入顺序：首页=》列表=》新增=》列表  ----堆栈会变为---->  首页=》列表
                    //页面进入顺序：首页=》新增=》列表        ----堆栈会变为---->  首页=》新增=》列表
    ...
    handleTextChange = (key, text) => {
        const { phoneList } = this.state
        if (phoneList[key].phone === text.slice(0, text.length - 1)) {
            text = text.replace(/\D$/, '*')
        }
        phoneList[key].phone = text
        phoneList[key].status = /\d{11}/.test(phoneList[key].phone)
    ...
    render() {
        ...
        const { selectCode, showMatching, isMan, phoneList, isEdit, ageChooseValue, buildTypeChooseValue,
            ...
            chooseResidentArea, dataInfo, mark } = this.state;
        return (
            <BaseContainer
            ...
            >
                <ScrollView ref={ref => this.scrollRef = ref}>
                            ...
                            <Input
                                ...
                                rightContent={
                                    <View style={styles.sexBet}>
                                        <TouchableOpacity style={[styles.sexView, isMan && { backgroundColor: '#1F3070', borderWidth: 0 }]} onPress={() => this.chooseSex(1)}>
                                            <Text style={[styles.sexText, isMan && { color: '#fff' }]}>男</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={[styles.sexView, !isMan && { backgroundColor: '#1F3070', borderWidth: 0 }]} onPress={() => this.chooseSex(0)}>
...

                    {
                        phoneList.map((item) => (
                            <Input
                                elem={ref => this[`phone${item.key}`] = ref}
                                label={
                                    <Text style={[styles.text, { width: scaleSize(180) }]}>手机号</Text>
                                }
                                style={{ textAlign: 'right'}}
                                placeholder='支持半号码，隐号请用任意符号代替'
                                maxLength={11}
                                onChange={text => this.handleTextChange(item.key, text)}
                                ...
                                rightContent={
                                    item.key === 0 ?
                                        ...
                                        <TouchableOpacity onPress={() => this.deleteOne(item)}>
                                            <Image source={DELETE} style={{ width: scaleSize(30), height: scaleSize(30), marginRight: scaleSize(32),marginLeft:scaleSize(16) }} />
                                        </TouchableOpacity>
                                }
                            >
                                {item.phone}
                    ...
                    <Animated.View style={[styles.inputWrap, { paddingBottom: this.state.keyboardHeight }]} >
                        <TextInput
