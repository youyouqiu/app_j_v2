...
import {checkPermission, customerNameSubstring, extractIdFromUrl, updateUrlParams, verifyUser} from '@/utils/utils'
...
import tracking, {BehaviorLog, PageTimer} from '../../utils/BuryPoint'
...
class Workbench extends PureComponent<NavigationScreenProps & TStateProps & DispatchProp, State> {
    ...
    componentDidUpdate(prevProps: TStateProps) {
        ...
        //     this.readPush(noticeInfo.pushId)
        //     noticeInfo.url && navigationUtils.noticeNavigate(noticeInfo)
        // }
        if (prevProps.location.locationCityCode !== this.props.location.locationCityCode) {
            this.getADOfOpen()
        }
    }
    ...
    putRead = (conditions: ReadConditions) => {
        return messageApi.readDt(conditions)
    }

    // 点击PUSH已读信息
    readPush = async (id?: string) => {
        await messageApi.read({pushId: id})
        this.props.dispatch({type: 'getLastNews/getList'}) // 进行一次数据采集。进行外部角标设置
    }
    ...
    getADOfHeadline = async () => {
        ...
        try {
            ...
            console.log(extension, 'conditions-头词条')
        } catch (e) {
    ...
    getADOfOpen = async () => {
        ...
        try {
            ...
            //预加载
            Image.prefetch(ADOnOpen[0].cover).then((a: boolean) => {
                console.log(ADOnOpen[0].cover)
                this.setState({ADOnOpen}, () => a && this.props.dispatch({type: 'config/controlADVisible'}))
            })
        } catch (e) {
    ...
    handlePressCustomerDynamic = async () => {
        ...
        const {guest, navigation, dispatch} = this.props
        ...
        await this.putRead(conditions).catch(e => console.log('read error:', e))
        dispatch({type: 'getLastNews/getList'}) // 进行一次数据采集。进行外部角标设置
        navigation.navigate('messageDetail', {type: 15, init: this.getDtInfo})
    };
    ...
    gotoWebView = (item: GetAdvertisingsResponseExtensionListItem) => {
        ...
        if (item.jumpType === 0) {
            item.link = updateUrlParams(item.link, [{key: 'source', value: 'KaiPing'}]);
            item.source = 'KaiPing';
    ...
    handleRefresh = () => {
      ...
      this.props.dispatch({type: 'location/getLocationInfo'})
        this.props.dispatch({type: 'user/interfaceUpdateUserAsync', payload: null}) // 刷新用户信息数据
    ...
    renderBackLogContentRightText = ({messageType, dataContent}: WaitMessageResponseListItem) => {
        const {day, buildingName, shopName, clientName, clientPhone, hour, clientNameAndPhone = ''} = JSON.parse(dataContent);
        switch (messageType) {
            case 'RemindNotSign': {
                return (
                    <>
                        <Text style={styles['blued']}>
                            {clientName ? `${customerNameSubstring(clientName, 3)}(${clientPhone})` : clientNameAndPhone}
                        </Text>在
    ...
    render() {
        ...
        return <>
            ...
            >
                ...
                {headline.length ? (
                    <View style={styles['headlines']}>
                        ...
                        <VerticalSwiper style={styles['headlines-content']}>
                            {headline.map(item => (
                                <TouchableOpacity key={item.id} activeOpacity={0.8} onPress={() => this.handleHeadLine(item)}
                                                  style={{height: scaleSize(66), flexDirection: 'row', alignItems: 'center'}}>
                                    <Text style={[styles['headlines-text'], {flex: 1}]} numberOfLines={2}>{item.adName}</Text>
                ...
                <EntryIcons>
                    <EGroup title='常用功能'>
                        ...
                        <EItem title='聚焦热点' path='articleList' auth={true} icon={require('./../../images/icons/entryIcon/zxgh2.png')}/>
                    </EGroup>
                    <EGroup title='工具助手'>
                        ...
                        <EItem title='侦探寻铺' path='searchBuilding' icon={require('./../../images/icons/entryIcon/ztxf.png')}/>
                    </EGroup>
                </EntryIcons>
            </ScrollView>
            ...
            {/*{*/}
            {/*    ADOnOpen.length >= 1 &&*/}
            {/*    <Advertisement images={ADOnOpen} onPress={(item: any) => this.handlePressADOnOpen(item)}/>*/}
            {/*}*/}
        </>
    }
}
