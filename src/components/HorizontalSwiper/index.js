...
export default class MarqueeHorizontal extends Component {
    ...
    textOnLayout = (e) => {
        ...
        let { textList, separator, children } = this.props;
        let length = textList.length || children.length
        this.setState({
            textWidth : width + ((length - 1) * separator),
        })
    }
    ...
    textView(list){
        let { textStyle, onTextClick, reverse, separator, children } = this.props;
        let itemView = children || [];
        if (!itemView) {
    ...
    textLengthView(list){
        let { textStyle, children } = this.props;
        let text = '';
        for(let i = 0;i<list.length;i++){
            text += list[i].value;
        }
        let length = list.length || children.length
        return(
            <View style = {{
